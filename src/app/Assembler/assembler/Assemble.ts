import { AddressingMode, InstructionInfo } from "../instructions"
...
export class ParseNodeHandler {
  ...
  static command (node, scope) {
    ...
    return new Directive({ line, name, params })
  }
...
function evaluateCommands (linearForms) {
  return linearForms.map(lir => {
    if (!(lir instanceof Directive) || lir.name === 'org') {
      return lir
    }
 ...
 *   label references.
 */
function resolveLabels ({ local, global }, instruction: Instruction) {
 ...
 */
export function buildInstruction (instruction) {
 ...
 */
export function assemble (rootNode) {
  
  // Recursively construct the linear representation of the given root node
  const scope = new Scope()
  let linearForm: Array<any> = assembleParseNode(rootNode, scope)
  ...
  linearForm.filter(lir => lir instanceof Label).forEach((label: Label) => {
  ...
  linearForm.filter(lir => lir instanceof Instruction).forEach((inst: Instruction) => {
    resolveLabels(labelScope, inst)
    buildInstruction(inst)
  })

  // realocate the addres given by the label at .addr command
  linearForm.map(lir=>{
    if(lir instanceof ByteArray){ 
      const params = lir.line._data.assembly.split(" ");
      if(params[0] === ".addr")
      {
        const address = labelScope.global.get(params[1]);
